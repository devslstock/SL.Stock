...
export function SalesDashboard() {
  ...
  const objetivoNoMes = 0 // Mocked goal, as discussed
  const missingGoal = Math.max(0, objetivoNoMes - vendidoNoMes)
  
  // Calcula dias úteis (simplificado para exemplo)
  ...
  // Evolução de Venda (Agrupado por Dia)
  ...
  const carteiraData = [
    { name: 'Ativos', value: ativosCount, color: '#10b981' }, // emerald-500
    { name: 'Inativos', value: inativosCount, color: '#ef4444' } // red-500
  ]

  // Positivação (Clientes Ativos que compraram no mês vs Todos os Ativos)
  ...
  const positivacaoData = [
    ...
    { name: 'Não Positivados', value: naoPositivadosCount, color: '#f59e0b' } // amber-500
  ]

  // Curva ABC
  // Sum net amount per customer
  ...
  salesByCustomer.forEach(item => {
    ...
    const percentage = (accumulatedForABC / totalSales) * 100
  ...
  const abcData = [
    { name: 'Curva A', value: curvaA, color: '#8b5cf6' }, // violet-500
    { name: 'Curva B', value: curvaB, color: '#ec4899' }, // pink-500
    { name: 'Curva C', value: curvaC, color: '#3b82f6' }  // blue-500
  ]
