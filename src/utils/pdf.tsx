...
export async function drawDeliveryProofOnDoc(doc: jsPDF, client: any, company: any) {
  ...
  if (!orderDetails) {
    orderDetails = {
      order_number: client.order_number || client.id,
      created_at: client.created_at,
      customer: {
        legal_name: client.name || 'Cliente Sem Nome',
        document: client.document || client.customer?.document,
      ...
      items: (client.delivery_items || []).map((i: any) => ({
        id: i.id,
        quantity: i.quantity_expected,
        unit_price: 0,
        total_price: 0,
        discount_percent: 0,
        product: { description: i.description, unit: i.unit || 'UN' }
      })),
      total_discount: 0,
      notes: client.notes,
  ...
  await new Promise<void>((resolve) => {
    root.render(
      <div id={`pdf-wrapper-${client.id}`}>
        <InvoicePrintTemplate details={orderDetails} company={company} />
      </div>
    );
    
    setTimeout(async () => {
      const node = document.getElementById(`pdf-wrapper-${client.id}`);
      if (node) {
        try {
          ...
          doc.text('Erro ao gerar layout do comprovante.', 15, 20);
        }
      }
      root.unmount();
...
export async function generateDeliveryProofPDF(client: any, company: any): Promise<void> {
  ...
  await drawDeliveryProofOnDoc(doc, client, company)
