...
export default function ReceiptMethods() {
  ...
  const filteredMethods = methods.filter((method: ReceiptMethod) => {
    const matchesSearch = method.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          method.holder_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          method.bank?.toLowerCase().includes(searchTerm.toLowerCase())
  ...
  const handleToggleStatus = (method: ReceiptMethod) => {
    ...
    if (newStatus === 'Inativo') {
      if (!confirm(`Tem certeza que deseja inativar a forma de recebimento "${method.name}"? Ela não poderá mais ser usada em novos lançamentos.`)) {
        return
      }
    }
  ...
  const formatPixKey = (key: string | undefined | null) => {
    if (!key) return '-'
      ...
      <div className="border-b bg-white px-6 py-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button onClick={() => { setEditingMethod(null); setIsModalOpen(true); }} className="bg-primary hover:bg-primary/90 text-white gap-2">
            <Plus className="h-4 w-4" /> Nova forma de recebimento
          </Button>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <Input
              placeholder="Pesquisar por nome, banco ou titular..."
              value={searchTerm}
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-50/50 border-b">
                <tr>
                  ...
                  <th className="px-4 py-3 font-medium text-gray-600">Banco / Detalhes</th>
                  <th className="px-4 py-3 font-medium text-gray-600">Titular</th>
              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="px-4 py-8 text-center text-gray-500">Nenhuma forma de recebimento encontrada.</td>
                  </tr>
                ) : (
                  filteredMethods.map((method: ReceiptMethod) => (
                    <tr key={method.id} className="border-b last:border-0 hover:bg-gray-50/50">
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-gray-100 text-gray-700 uppercase">
                          {method.type}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-gray-600">
                        {method.type === 'banco' ? (
                          <span>{method.bank} - Ag {method.agency} Cc {method.account_number}-{method.account_digit}</span>
                        ) : method.type === 'pix' ? (
                          <span>{method.linked_bank || 'PIX'} - {formatPixKey(method.pix_key)}</span>
                        ) : (
                          <span>{method.notes || '-'}</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-gray-600">{method.holder_name || '-'}</td>
                      <td className="px-4 py-3">
