...
export function ProductSearchInline({ priceTableId, customerId, currentItems, onUpdateQuantity }: ProductSearchInlineProps) {
  ...
  return (
    <div className="flex flex-col bg-card border border-border rounded-xl shadow-sm overflow-hidden md:h-[600px] md:max-h-[70vh] h-full min-h-[85vh] md:min-h-0">
      {/* TABS */}
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative h-11 w-11 shrink-0 flex items-center justify-center border border-border rounded-md bg-muted/10 hover:bg-muted/30 transition-colors">
            ...
            >
              ...
              {categories.map(c => <option className="bg-background text-foreground" key={c} value={c}>{c}</option>)}
            </select>
            {selectedCategory && (
