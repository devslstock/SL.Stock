...
export default function AddRouteClient() {
  ...
  const importMutation = useMutation({
    mutationFn: (clientData: any[]) => deliveriesApi.importDeliveryClients(id!, clientData),
    onSuccess: () => {
  ...
  if (!isManager) {
    return <Navigate to={`/entregas/${id}`} replace />
  }

  if (isLoadingRoute) return <div className="p-8 text-center text-muted-foreground">Carregando rota...</div>

  return (
    <div className="space-y-6 max-w-2xl mx-auto pb-20 slide-in">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" onClick={() => navigate(`/entregas/${id}`)}>
          <ArrowLeft className="h-5 w-5" />
        </Button>
        <div>
          <h1 className="text-2xl font-bold gradient-text">Novo Cliente</h1>
          <p className="text-sm text-muted-foreground">Adicionar avulso na Rota: {route?.title || 'Sem título'}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          {importMutation.isPending ? 'Salvando...' : <><Save className="h-6 w-6" /> Salvar Cliente na Rota</>}
        </Button>
      </form>
    </div>
  )
